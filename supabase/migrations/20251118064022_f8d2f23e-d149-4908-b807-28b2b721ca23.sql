-- Fix create_order_with_items to remove organization_id reference
DROP FUNCTION IF EXISTS public.create_order_with_items(uuid, uuid, order_type, uuid, uuid, numeric, numeric, numeric, numeric, jsonb, uuid, jsonb, jsonb);
...
DECLARE
  ...
  v_item jsonb;
BEGIN
  -- Insert order with kitchen_queue status and branch_id (NO organization_id)
  INSERT INTO orders (
