  ...
  return (
    <Card className={cn(
      ...
      <div className={cn(
        ...
        config.compactMode ? "space-y-1.5" : "space-y-2"
      )}>
        {isLoading ? (
          <>
          ...
          pendingMods.map((mod: any) => (
            <button
              ...
              className={cn(
                "w-full text-left rounded-lg border border-warning/20 bg-warning/5 transition-all hover:bg-warning/10 hover:border-warning/30",
                config.compactMode 
                  ? "p-2 min-h-[40px]" 
                  : "p-2.5 min-h-[48px]"
              )}
            >
              {config.compactMode ? (
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {config.showWastageCost && mod.wastage_cost > 0 && (
                    <Badge 
                      variant={getWastageBadgeVariant(mod.wastage_cost)}
                      className="text-xs whitespace-nowrap"
                    >
                      <DollarSign className="h-3 w-3 mr-0.5" />
                      {mod.wastage_cost.toFixed(2)}
                    </Badge>
                  )}
                </div>
              ) : (
                // Standard Mode: Multi-line
                <>
