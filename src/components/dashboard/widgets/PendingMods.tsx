...
export default function PendingMods() {
  ...
  const getWastageBadgeVariant = (cost: number) => {
  ...
  return (
    <Card className={cn("glass-card flex flex-col w-[360px] h-[300px]", config.compactMode ? "p-3" : "p-4")}>
      <div className={cn("flex items-center justify-between", config.compactMode ? "mb-2" : "mb-3")}>
        ...
        <Button onClick={() => refetch()} variant="ghost" size="sm" className="h-8 w-8 p-0"><RefreshCw className="h-4 w-4" /></Button>
      </div>
      ...
      <div className={cn("flex-1 overflow-y-auto min-h-0", config.compactMode ? "space-y-1.5" : "space-y-2")}>
        {isLoading ? (
          ...
          pendingMods.map((mod: any) => (
            <button key={mod.id} onClick={() => navigate('/admin/pending-modifications')} className={cn("w-full text-left rounded-lg border border-warning/20 bg-warning/5 transition-all hover:bg-warning/10 hover:border-warning/30", config.compactMode ? "p-2 min-h-[40px]" : "p-2.5 min-h-[48px]")}>
              {config.compactMode ? (
