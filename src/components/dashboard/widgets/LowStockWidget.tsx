      ...
      <div className={cn(
        ...
        {isLoading ? (
          ...
          lowStockItems.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full rounded-md border transition-all hover:shadow-sm active:scale-[0.98]",
                  config.compactMode ? "p-1.5" : "p-2",
                  status.bg,
                  status.animate ? "border-destructive/30" : "border-border/30"
                )}
              >
                <div className="flex items-center gap-2">
                  <div className={cn(
                    "flex items-center justify-center rounded-full flex-shrink-0",
                    config.compactMode ? "w-6 h-6" : "w-7 h-7",
                    status.animate ? "bg-destructive/20" : "bg-warning/20"
                  )}>
                    {status.animate ? (
                      <PackageX className={cn(
                        "text-destructive animate-pulse",
                        config.compactMode ? "h-3 w-3" : "h-3.5 w-3.5"
                      )} />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <p className={cn(
                      ...
                      config.compactMode ? "text-[9px]" : "text-[10px]"
                    )}>
                  ...
                  <div className={cn(
                    ...
                    status.animate ? "bg-destructive animate-pulse" : "bg-warning"
                  )} />
                </div>
              </button>
            );
          ...
          <div className="flex flex-col items-center justify-center text-muted-foreground">
            ...
            <p className={cn(
              "font-medium",
              config.compactMode ? "text-[10px]" : "text-xs"
            )}>
              All in stock
            </p>
          </div>
